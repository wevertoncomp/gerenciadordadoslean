...
$instrucaoSQL2 = "	SELECT 
					...
					Z9.ZZ9_KANBVM,
					Z9.ZZ9_QTDKAN,
					(SELECT SUM(B2.B2_QATU) FROM SB2010 B2 WHERE B2.B2_COD = B1.B1_COD AND (B2.B2_LOCAL LIKE 'PP%' OR B2.B2_LOCAL LIKE 'M_-%') AND B2.B2_LOCAL <> 'PP-TRA') AS EstoquePP,
					(SELECT SUM(B2.B2_QATU) FROM SB2010 B2 WHERE B2.B2_COD = B1.B1_COD AND B2.B2_LOCAL LIKE 'AL%' AND B2.B2_LOCAL <> 'AL-TRA') AS EstoqueAL,
					(SELECT SUM(B2.B2_QATU) FROM SB2010 B2 WHERE B2.B2_COD = B1.B1_COD AND B2.B2_LOCAL LIKE 'PVC%') AS EstoquePVC,
					(SELECT SUM(B2.B2_QATU) FROM SB2010 B2 WHERE B2.B2_COD = B1.B1_COD AND B2.B2_LOCAL LIKE 'Z%' AND B2.B2_LOCAL <> 'ZIN-TR') AS EstoqueZIN,
